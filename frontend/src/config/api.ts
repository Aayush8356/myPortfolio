...
export const BLOB_BASE_URL = process.env.REACT_APP_BLOB_BASE_URL ||
  (process.env.NODE_ENV === 'development' ? 'http://localhost:5002/blob' : `${RUNTIME_ORIGIN}/blob`);

export const ASSETS_BASE_URL = process.env.REACT_APP_ASSETS_BASE_URL ||
  (process.env.NODE_ENV === 'development' ? 'http://localhost:5002/assets' : `${RUNTIME_ORIGIN}/assets`);